...
class DownloadsFragment : Fragment(), DownloadFileClickListener {
            ...
            if (it.isNullOrEmpty()) {
                binding.apply {
                    ...
                    gotoHome.setOnClickListener{
                        (activity as MainActivity).findNavController(R.id.nav_host_fragment_activity_main).navigateUp()
                    }
                }
    ...
    private fun getDivider(): DividerItemDecoration {
        ...
        val drawable = requireContext().getDrawable(R.drawable.divider_drawable)
    ...
    override fun onDeleteClick(fileMeta: FileMeta, position: Int) {
        downloadsViewModel.getResolveInfo().observe(viewLifecycleOwner, {
            if(it.isNotEmpty()) {
                ...
                Snackbar.make(binding.root,"App is not installed yet", Snackbar.LENGTH_SHORT).show()
            }
        })
    }
